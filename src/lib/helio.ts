...
const PLANETS = [
  ...
  { body: Body.Earth, nameRu: "Земля", tone: "var(--accent)" },
  { body: Body.Mars, nameRu: "Марс", tone: "var(--planet-mars)" },
  { body: Body.Jupiter, nameRu: "Юпитер", tone: "var(--planet-jupiter)" },
...
export type HelioPos = {
  ...
  /** The Moon is not on a solar orbit and never gets one here: it is drawn as a
      satellite beside Earth. Its longitude and speed are geocentric, which is
      also the only frame where they mean anything. */
  isMoon: boolean;
};
